...
@Component({
  selector: 'app-chamados-infraestrutura',
  templateUrl: './chamados-infraestrutura.component.html',
  styleUrl: './chamados-infraestrutura.component.scss'
})
export class ChamadosInfraestruturaComponent implements OnInit, OnDestroy {
  ...
  protected servicosInfra: IServico[] = [];
  ...
  async ngOnInit(): Promise<void> {
    ...
    // 2) Filtra serviços da unidade Infraestrutura
    this.servicosInfra = this.servicos.filter(s =>
      (s.Unidade ?? '').trim().toLowerCase() === 'infraestrutura'
    );

    // Substitui dropdown principal para mostrar apenas Infra
    this.servicos = this.servicosInfra;
  ...
  protected async carregarTodosChamados(): Promise<void> {
    ...
    const idsServicosInfra = new Set(this.servicosInfra.map(s => s.ServicoID));

    const chamadosInfra = chamados.filter(c => c.ServicoID && idsServicosInfra.has(c.ServicoID));

    const usuarioMesmoDepartamento = this.usuarios.filter(u => u.DepartamentoID === this.departamentoID);

    this.todosChamadosUsuario = chamadosInfra.filter(chamado => {
