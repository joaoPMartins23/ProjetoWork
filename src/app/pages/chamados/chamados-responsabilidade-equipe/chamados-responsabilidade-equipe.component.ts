...
@Component({
  selector: 'app-chamados-responsabilidade-equipe',
  templateUrl: './chamados-responsabilidade-equipe.component.html',
  styleUrl: './chamados-responsabilidade-equipe.component.scss'
})
export class ChamadosResponsabilidadeEquipeComponent implements OnInit, OnDestroy {
  ...
  protected departamentoID: string = CryptoJS.AES
    .decrypt(localStorage.getItem('DepartamentoID') || '', environment.chavePrivada)
    .toString(CryptoJS.enc.Utf8);

  protected usuarioID: string = CryptoJS.AES
    .decrypt(localStorage.getItem('UsuarioID') || '', environment.chavePrivada)
    .toString(CryptoJS.enc.Utf8);
  ...
  protected cabecalhosEnviar: any[] = [
    ...
    { CampoTitulo: "Responsável", CampoChave: "Responsavel" },   // <- sem acento
    { CampoTitulo: "Serviço", CampoChave: "Servico" },       // <- sem acento
    { CampoTitulo: "Status", CampoChave: "Status" },
    { CampoTitulo: "Aberto em", CampoChave: "DataAbertura" },  // <- nome interno
  ...
  private usuarios: IUsuarioView[] = [];
  protected servicos: IServico[] = []; // <- usar no template para o filtro de Serviço
  ...
  // ===== ORDENACAO =====
  protected criterioOrdenacao: { campo: string | null; direcao: 'asc' | 'desc' } = {
    campo: null,
    direcao: 'desc'
  };
  ...
  private previewCache = new Map<string, string>(); // ChamadoID -> primeira mensagem
  ...
  async ngOnInit(): Promise<void> {
    ...
    // 1) Lê filtros e página via queryParams (APENAS UMA VEZ)
    this.activatedRoute.queryParams.subscribe(params => {
      ...
      // Depois do carregamento inicial, refiltra sempre que os params mudarem
      if (!this.carregando) {
        this.aplicarFiltro();
      }
    });

    // 2) Carrega tudo em paralelo: chamados + dados auxiliares
    const [_, servicos, usuarios, deps] = await Promise.all([
      this.carregarTodosChamados(), // já preenche this.todosChamadosUsuario
      this.servicoService.buscarServicos().toPromise(),
      this.usuarioService.buscarUsuarios().toPromise(),
      this.departamentoResponsavelService.buscarDepartamentosResponsaveis().toPromise()
    ]);
    ...
    this.departamentosResponsaveis = deps || [];

    // 3) Filtra usuários do departamento do usuário logado
    this.usuariosDepartamento = this.usuarios.filter(
      u => u.DepartamentoID === this.departamentoID
    );

    // 4) Aplica filtro inicial com tudo carregado
    this.aplicarFiltro();
    this.carregando = false;

    // 5) Atualização periódica (1 min)
  ...
  protected async carregarTodosChamados(): Promise<void> {
    ...
    const usuarios: IUsuarioView[] =
      await this.usuarioService.buscarUsuarios().toPromise() || [];

    const usuariosMesmoDepartamento = usuarios.filter(u => u.DepartamentoID === this.departamentoID);
    const idsUsuariosMesmoDepartamento = usuariosMesmoDepartamento.map(u => u.UsuarioID);

    this.todosChamadosUsuario = chamados
      .filter(ch => !(ch as any)?.ClienteID)
      .filter(ch =>
        idsUsuariosMesmoDepartamento.includes(ch.UsuarioResponsavelID) &&
        ch.UsuarioResponsavelID !== this.usuarioID
      );
  }
  ...
  private normalizarCampo(titulo: string): string {
  ...
  private compararValores(a: any, b: any, campoChave: string): number {
    if (campoChave === 'DataAbertura') {
      return this.parseDataBR(a) - this.parseDataBR(b);
    }
    if (campoChave === 'Prioridade') {
      const rank: Record<string, number> = { 'Alta': 3, 'Média': 2, 'Baixa': 1 };
      return (rank[a] || 0) - (rank[b] || 0);
    }
    const na = typeof a === 'number' ? a : Number(a);
    const nb = typeof b === 'number' ? b : Number(b);
    const aNum = !isNaN(na) && a !== '' && a !== null && a !== undefined;
    const bNum = !isNaN(nb) && b !== '' && b !== null && b !== undefined;
    if (aNum && bNum) return na - nb;

    return String(a ?? '').toLowerCase().localeCompare(String(b ?? '').toLowerCase());
  }

  // chamada no (click) do <th>
  ordenarPor(campoTitulo: string): void {
    if (this.criterioOrdenacao.campo === campoTitulo) {
      this.criterioOrdenacao.direcao =
        this.criterioOrdenacao.direcao === 'asc' ? 'desc' : 'asc';
    } else {
      this.criterioOrdenacao.campo = campoTitulo;
      this.criterioOrdenacao.direcao = campoTitulo === 'Prioridade' ? 'desc' : 'asc';
    }
    this.aplicarFiltro();
  }

  // dispara no (change) do select de Serviço no template
  ...
  private updatePreviewPos(ev: MouseEvent): void {
    const PADDING = 12;
    const CARD_W = 520;   // estimativa da largura do card
    const CARD_H = 220;   // estimativa da altura do card
    let x = ev.clientX + 16;
    let y = ev.clientY + 16;
  ...
  /**
   * Busca a primeira mensagem do chamado.
   * Ajuste o método abaixo para o nome do seu endpoint real.
   */
  private async buscarPrimeiraMensagem(chamadoId: string): Promise<string> {
    // Opção A: endpoint que traz o chamado completo (com mensagens)
    // const detalhado = await this.chamadoService.buscarChamado(chamadoId).toPromise();

    // Opção B: endpoint específico de mensagens
    // const msgs = await this.chamadoService.buscarMensagens(chamadoId).toPromise();

    // ---- Exemplo genérico (adapte ao que sua API retorna) ----
    ...
    // ordena por data crescente para pegar a primeira
    ...
    // texto/descrição pode estar em campos diferentes dependendo do backend
  ...
  onPesquisarTermo(valor: string): void {
    ...
    this.searchTimer = setTimeout(() => {
      if (this.destroyed) return; // <--- garante que já não morreu
