...
@Component({
  selector: 'app-chamados-monitoramento',
  templateUrl: './chamados-monitoramento.component.html',
  styleUrl: './chamados-monitoramento.component.scss'
})
export class ChamadosMonitoramentoComponent implements OnInit, OnDestroy {
  ...
  protected servicosMonitoramento: IServico[] = [];
  ...
  async ngOnInit(): Promise<void> {
    ...
    // 2) Filtra serviços da unidade Monitoramento
    this.servicosMonitoramento = this.servicos.filter(s =>
      (s.Unidade ?? '').trim().toLowerCase() === 'monitoramento'
    );
    this.servicos = this.servicosMonitoramento;

    // 3) Lê filtros e página via queryParams (APENAS UMA VEZ)
    this.activatedRoute.queryParams.subscribe(params => {
      if (this.destroyed) return;

      this.filtroSelecionado = params['status'] || 'todos';  // mantém seu default
  ...
  protected async carregarTodosChamados(): Promise<void> {
    ...
    const idsServicosMonitoramento = new Set(this.servicosMonitoramento.map(s => s.ServicoID));
    const chamadosMonitoramento = chamados.filter(c => c.ServicoID && idsServicosMonitoramento.has(c.ServicoID));

    const usuarioMesmoDepartamento = this.usuarios.filter(u => u.DepartamentoID === this.departamentoID);

    this.todosChamadosUsuario = chamadosMonitoramento.filter(chamado => {
  ...
  private updatePreviewPos(ev: MouseEvent): void {
    ...
    if (y + CARD_H > window.innerHeight) y = ev.clientY - CARD_H - PADDING;
  ...
  private async buscarPrimeiraMensagem(chamadoId: string): Promise<string> {
    ...
    const sorted = [...mensagens].sort((a, b) => new Date(a.DataHoraInicio || a.DataHora || a.dataHora).getTime() - new Date(b.DataHoraInicio || b.DataHora || b.dataHora).getTime());
