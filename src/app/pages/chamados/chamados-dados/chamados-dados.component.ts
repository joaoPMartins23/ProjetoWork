...
@Component({
  selector: 'app-chamados-dados',
  templateUrl: './chamados-dados.component.html',
  styleUrl: './chamados-dados.component.scss'
})
export class ChamadosDadosComponent implements OnInit {
  ...
  protected servicosDados: IServico[] = [];
  ...
  async ngOnInit(): Promise<void> {
    ...
    // 2) Filtra serviços da unidade Dados
    this.servicosDados = this.servicos.filter(s =>
      (s.Unidade ?? '').trim().toLowerCase() === 'dados'
    );

    // Substitui dropdown principal para mostrar apenas Dados
    this.servicos = this.servicosDados;
  ...
  protected async carregarTodosChamados(): Promise<void> {
    ...
    const idsServicosDados = new Set(this.servicosDados.map(s => s.ServicoID));

    const chamadosDados = chamados.filter(c => c.ServicoID && idsServicosDados.has(c.ServicoID));

    const usuarioMesmoDepartamento = this.usuarios.filter(u => u.DepartamentoID === this.departamentoID);

    this.todosChamadosUsuario = chamadosDados.filter(chamado => {
