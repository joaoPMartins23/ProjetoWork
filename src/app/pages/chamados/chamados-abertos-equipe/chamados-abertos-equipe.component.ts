...
@Component({
  selector: 'app-chamados-abertos-equipe',
  templateUrl: './chamados-abertos-equipe.component.html',
  styleUrl: './chamados-abertos-equipe.component.scss'
})
export class ChamadosAbertosEquipeComponent implements OnInit, OnDestroy {
  ...
  protected servicos: IServico[] = []; // <- usar no filtro por serviço
  ...
  async ngOnInit(): Promise<void> {
    ...
    this.activatedRoute.queryParams.subscribe(params => {
      ...
      // Só refiltra automaticamente DEPOIS que o carregamento inicial terminar
      if (!this.carregando) {
        this.aplicarFiltro();
      }
    });

    // 2) Carrega dados necessários em paralelo
    const [servicos, usuarios] = await Promise.all([
    ...
    this.intervaloAtualizacao = setInterval(async () => {
      ...
      await this.carregarTodosChamados(); // rebusca do backend
      if (this.destroyed) return;

      this.aplicarFiltro();               // reaplica filtros/pesquisa atuais
    }, 180000);
  }
  ...
  protected async carregarTodosChamados(): Promise<void> {
    ...
    this.departamentosResponsaveis =
    ...
    this.todosChamadosUsuario = base.filter(chamado =>
      idsUsuariosMesmoDepartamento.includes(chamado.UsuarioCriacaoID) &&
  ...
  // chamada no (click) do <th> no template
