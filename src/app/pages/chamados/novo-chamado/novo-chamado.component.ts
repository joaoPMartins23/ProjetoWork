...
export class NovoChamadoComponent implements OnInit {
  ...
  protected departamentoIDArmazenado = localStorage.getItem('DepartamentoResponsavelID') || '';

  protected departamentoIDUsuario: string = (() => {
    const raw = this.departamentoIDArmazenado;
    if (!raw) return '';
    try {
      const dec = CryptoJS.AES.decrypt(raw, environment.chavePrivada).toString(CryptoJS.enc.Utf8);
      return dec || raw; // se decifrar vazio, usa cru
    } catch {
      return raw; // já estava cru
    }
  })();
  ...
  async ngOnInit(): Promise<void> {
    ...
    try {
      ...
      this.formularioChamado.get('DepartamentoResponsavelID')?.valueChanges.subscribe(departamentoID => {
        if (!departamentoID) {
        ...
        const podeEscolherResponsavel = departamentoID === this.departamentoIDUsuario;
        ...
        // (continua sua lógica de preencher listas)
  ...
  onSubmit(): void {
    if (this.formularioChamado.valid) {
      ...
      const textoOriginal: string = this.formularioChamado.value.Mensagem || '';
      const textoComCRLF = textoOriginal.replace(/\r?\n/g, '\r\n');

      const chamado: IChamadoForm = {
        Titulo: this.formularioChamado.value.Titulo,
        Prioridade: this.formularioChamado.value.Prioridade,
        DataHora: this.getLocalISODateString(new Date()),
        DataHoraFinalizacao: null,
        UsuarioFinalizacaoID: null,
        BlocoID: this.formularioChamado.value.BlocoID != '' ? this.formularioChamado.value.BlocoID : null,
        UsuarioCriacaoID: this.formularioChamado.value.UsuarioCriacaoID == "" ? CryptoJS.AES.decrypt(localStorage.getItem('UsuarioID') || '', environment.chavePrivada).toString(CryptoJS.enc.Utf8) : this.formularioChamado.value.UsuarioCriacaoID,
        DepartamentoResponsavelID: this.formularioChamado.value.DepartamentoResponsavelID,
        UsuarioResponsavelID: this.formularioChamado.value.UsuarioResponsavelID != '' ? this.formularioChamado.value.UsuarioResponsavelID : null,
        Status: 'Em espera',
        Mensagens: {
          UsuarioID: CryptoJS.AES.decrypt(localStorage.getItem('UsuarioID') || '', environment.chavePrivada).toString(CryptoJS.enc.Utf8),
          ...
          Arquivos: this.arquivosSelecionados
            ? this.arquivosSelecionados.map(a => ({
        ...
        ServicoID: this.formularioChamado.value.ServicoID,
        TipoServicoID: this.formularioChamado.value.TipoServicoID,
        // Estava dando erro quando o array era null, ele não reconhecia o map, Leonardo alterou para sempre ser um array vazio quando null
        //UsuariosAuxiliares: this.formularioChamado.value.AuxiliaresIDs ? this.formularioChamado.value.AuxiliaresIDs.map((usuarioID: string) => ({ UsuarioID: usuarioID })) : [],
        UsuariosAuxiliares: (this.formularioChamado.value.AuxiliaresIDs ?? [])
          .map
          ? this.formularioChamado.value.AuxiliaresIDs.map((usuarioID: string) => ({ UsuarioID: usuarioID }))
          : [],
        UsuarioRealizacaoID: this.usuarioID,
        ProjetoID: this.formularioChamado.value.ProjetoID != '' ? this.formularioChamado.value.ProjetoID : null,
        DataHoraInicioProjeto: this.formularioChamado.value.DataHoraInicioProjeto != '' ? this.formularioChamado.value.DataHoraInicioProjeto : this.getLocalISODateString(new Date()),
        DataHoraFinalizacaoProjeto: null,
        DataHoraEstimado: this.formularioChamado.value.DataHoraEstimado != '' ? this.formularioChamado.value.DataHoraEstimado : null
      }
      ...
      this.chamadoService.criarChamado(chamado).subscribe({
        next: async (response) => {
          ...
          // IDs dos auxiliares vindos do form (já vai no payload que você montou)
          ...
          if (responsavelID !== null) {
            this.notificacaoService.criarNotificacao({
              ...
              UrlDestino: `Chamados/Detalhes/${response.ChamadoID}`,
          ...
          this.router.navigate([`Chamados/Detalhes/${response.ChamadoID}`]);
        },
        error: (error) => {
  ...
  protected getPdfSafeUrl(): SafeResourceUrl {
    // ex.: '#zoom=page-width' ou '#toolbar=1'
    // Se quiser: return this.sanitizer.bypassSecurityTrustResourceUrl(`${this.preview.src}#zoom=page-width`);
  ...
  private async processFiles(files: File[]) {
    ...
    sessionStorage.setItem(this.DRAFT_KEY, JSON.stringify(this.formularioChamado.getRawValue()));
  }
  ...
  private getLimitForKind(kind: ReturnType<NovoChamadoComponent['getPreviewKind']>) {
    if (!kind) return null;
