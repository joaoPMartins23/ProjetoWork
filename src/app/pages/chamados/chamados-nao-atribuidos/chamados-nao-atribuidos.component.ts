...
@Component({
  ...
  templateUrl: './chamados-nao-atribuidos.component.html',
  styleUrl: './chamados-nao-atribuidos.component.scss'
})
export class ChamadosNaoAtribuidosComponent implements OnInit, OnDestroy {
  ...
  protected usuarioID = CryptoJS.AES.decrypt(localStorage.getItem('UsuarioID') || '', environment.chavePrivada).toString(CryptoJS.enc.Utf8);
  ...
  protected cabecalhosEditadosEnviar = [
  ...
  protected filtroSelecionado = 'nao-resolvidos';
  protected termoPesquisa = '';
  protected servicoSelecionadoId = 'todos';
  ...
  async ngOnInit(): Promise<void> {
    ...
    // 3) Aplica filtro inicial com tudo carregado
    this.aplicarFiltro();
  ...
  protected async carregarTodosChamados(): Promise<void> {
    ...
    this.todosChamadosUsuario = chamados
      ...
      .filter(ch =>
        !ch.UsuarioResponsavelID &&
        ch.DepartamentoResponsavelID === this.departamentoResponsavel
      );
  }
  ...
  private parseDataBR(dataStr: string): number {
    ...
    return new Date(ano, (mes || 1) - 1, dia || 1, hora || 0, min || 0).getTime();
  }
