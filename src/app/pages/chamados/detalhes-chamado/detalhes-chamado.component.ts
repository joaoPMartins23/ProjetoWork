...
@Component({
  ...
  styleUrl: './detalhes-chamado.component.scss'
})
export class DetalhesChamadoComponent implements OnInit {
  ...
  protected departamentoIDUsuario: string = (() => {
    const raw = localStorage.getItem('DepartamentoResponsavelID') || '';
    if (!raw) return '';
    try {
  ...
  private async inicializarFormulario(): Promise<void> {
    ...
    try {
      ...
      this.aplicarPermissaoEdicaoPorDepartamento();

      this.formularioChamado.get('ServicoID')?.valueChanges
        .pipe(takeUntil(this.destroy$))
        .subscribe((servicoId: string) => {
          const depChamado = this.formularioChamado.get('DepartamentoResponsavelID')?.value
            || this.chamadoAtual?.DepartamentoResponsavelID;

          const podeEditar = !!depChamado && depChamado === this.departamentoIDUsuario;

          if (!podeEditar) {
            this.formularioChamado.get('TipoServicoID')?.disable({ emitEvent: false });
            return;
          }
          this.formularioChamado.get('TipoServicoID')?.reset({ emitEvent: false });
      ...
      if (this.chamadoAtual.DepartamentoResponsavelID !== this.departamentoIDUsuario) {
        this.formularioChamado.get('UsuarioResponsavelID')?.reset();
        this.formularioChamado.get('UsuarioResponsavelID')?.disable();
      }

      if (this.chamadoAtual.ServicoID === null || this.chamadoAtual.ServicoID === '') this.formularioChamado.get('TipoServicoID')?.disable();

      this.formularioChamado.get('DepartamentoResponsavelID')?.valueChanges
        .pipe(takeUntil(this.destroy$))
        .subscribe(depId => {
          // 1. Atualiza listas dependentes
          this.servicosPorDepartamento = this.servicos
            .filter(s => s.DepartamentoResponsavelID === depId)
            .sort((a, b) => a.Servico.localeCompare(b.Servico));
          ...
          // 2. Limpa campos dependentes
          this.formularioChamado.get('UsuarioResponsavelID')?.setValue('', { emitEvent: false });
          ...
          // 3. O PULO DO GATO: Usa um timer para garantir que o Angular 
          // termine de processar a escolha antes de desabilitar o campo.
          setTimeout(() => {
            this.aplicarPermissaoEdicaoPorDepartamento();
          });
        });
      ...
      if (this.perfilAcessoUsuario === 'Usuário') {
        ...
        this.formularioChamado.get('UsuarioResponsavelID')?.disable();
        //this.formularioChamado.get('Status')?.disable();
      }
  ...
  private desabilitarCamposFormulario(): void {
    Object.keys(this.formularioChamado.controls).forEach(campo => {
      if (campo !== 'Status') this.formularioChamado.get(campo)?.disable();
    });

    // Garantia explícita (opcional):
      ...
      const notificacoesCriticas = destinatarios.map(destId =>
        this.safeCritical(
          this.notificacaoService.criarNotificacao({
            ...
            UsuarioID: destId,
            UrlDestino: `Chamados/Detalhes/${this.chamadoID}`,
          ...
          this.sendWhatsAppOnce(
            ...
            this.whatsappService.enviarMensagemWhatsApp({
              ...
              BodyParameters: [
                ...
                `https://work.bebidasprojeto.com.br/Chamados/Detalhes/${this.chamadoID}`
              ]
            }),
            30000,
 ...
 */
  ...
  // ===== Filtros de tipos que  já trata no preview =====
  ...
  protected projetoTempo(): {
    ...
    // Se terminou, compara com fim; se não, com agora
    const ref = fim || agora;
  ...
  private getLimitForKind(kind: ReturnType<DetalhesChamadoComponent['getPreviewKind']>) {
    if (!kind) return null;
  ...
  private aplicarPermissaoEdicaoPorDepartamento(): void {
    const depSelecionado = this.formularioChamado.get('DepartamentoResponsavelID')?.value;
    const ehDono = !!depSelecionado && depSelecionado === this.departamentoIDUsuario;
    const status = this.formularioChamado.get('Status')?.value;
    ...
    controles.forEach(c => {
      const ctrl = this.formularioChamado.get(c);
      if (!ctrl) return;

      // Se o chamado acabou, trava tudo sempre.
      if (encerrado) {
        ctrl.disable({ emitEvent: false });
        return;
      }

      // Se o usuário selecionou um departamento que não é o dele:
      if (!ehDono) {
        // Bloqueia o campo. 
        // OBS: Ele só conseguirá habilitar de novo se recarregar ou se 
        // houver um botão de "Desfazer".
        ctrl.disable({ emitEvent: false });
      } else {
  ...
  protected podeVisualizarMensagem(mensagem: IMensagemView): boolean {
    // Se não for interna, todos veem
    if (!mensagem.Interno) return true;

    // Se for interna, busca os dados do autor da mensagem
    const autor = this.buscarUsuarioMensagem(mensagem.UsuarioID);

    // Regra de ouro: Vejo se eu sou o autor OU se o meu departamento é igual ao do autor
    return mensagem.UsuarioID === this.usuarioID ||
      autor.DepartamentoResponsavelID === this.departamentoIDUsuario;
  }
}
