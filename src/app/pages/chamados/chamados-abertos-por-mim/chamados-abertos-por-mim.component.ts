...
@Component({
  selector: 'app-chamados-abertos-por-mim',
  templateUrl: './chamados-abertos-por-mim.component.html',
  styleUrl: './chamados-abertos-por-mim.component.scss'
})
export class ChamadosAbertosPorMimComponent implements OnInit, OnDestroy {
  ...
  private readonly HOVER_DELAY = 1000; // 1000ms = 1 segundo
  ...
  async ngOnInit(): Promise<void> {
    ...
    const [chamados, servicos, usuarios, depsResp] = await Promise.all([
      this.chamadoService.buscarChamados().toPromise(),
    ...
    if (this.destroyed) return; // se o usuário já saiu, não faz mais nada

    this.todosChamadosUsuario = (chamados || [])
      .filter(ch => !(ch as any)?.ClienteID)
      .filter(ch => ch.UsuarioCriacaoID === this.usuarioID);

    this.servicos = servicos || [];
    this.usuarios = usuarios || [];
    this.departamentosResponsaveis = depsResp || [];

    this.usuariosDepartamento = this.usuarios.filter(
      u => u.DepartamentoID === this.departamentoID
    );

    // 2) Um único subscribe para os query params
    this.activatedRoute.queryParams.subscribe(params => {
      ...
      // sempre que os params mudam, refiltra
      this.aplicarFiltro();
    });
    ...
    this.intervaloAtualizacao = setInterval(async () => {
      ...
      const chamadosAtualizados: IChamadoViewSimples[] =
        await this.chamadoService.buscarChamados().toPromise() || [];

      if (this.destroyed) return;

      this.todosChamadosUsuario = chamadosAtualizados
        .filter(ch => !(ch as any)?.ClienteID)
        .filter(ch => ch.UsuarioCriacaoID === this.usuarioID);
  ...
  limparTodosFiltros(): void {
    // valores padrão
    this.filtroSelecionado = 'nao-resolvidos';
