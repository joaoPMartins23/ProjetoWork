...
@Component({
  selector: 'app-chamados-desenvolvimento',
  templateUrl: './chamados-desenvolvimento.component.html',
  styleUrl: './chamados-desenvolvimento.component.scss'
})
export class ChamadosDesenvolvimentoComponent implements OnInit, OnDestroy {
  ...
  protected servicosDesenvolvimento: IServico[] = [];
  ...
  async ngOnInit(): Promise<void> {
    ...
    // 1) Carrega dados base em paralelo (serviços, usuários, departamentos)
    const [servicos, usuarios, departamentos] = await Promise.all([
    ...
    this.departamentosResponsaveis = departamentos || [];

    // 2) Filtra serviços da unidade Desenvolvimento
    this.servicosDesenvolvimento = this.servicos.filter(s =>
      (s.Unidade ?? '').trim().toLowerCase() === 'desenvolvimento'
    );

    // Substitui dropdown principal para mostrar apenas Desenvolvimento
    this.servicos = this.servicosDesenvolvimento;

    // 3) Lê filtros e página via queryParams (APENAS UMA VEZ)
    this.activatedRoute.queryParams.subscribe(params => {
      ...
      // se já terminou o carregamento inicial, refiltra quando params mudarem
      if (!this.carregando) {
        this.aplicarFiltro();
      }
    });
    ...
    // 5) Aplica filtro inicial
    this.aplicarFiltro();
    this.carregando = false;

    // 6) Atualização periódica (1 min)
  ...
  protected async carregarTodosChamados(): Promise<void> {
    const chamados: IChamadoViewSimples[] = await this.chamadoService.buscarChamados().toPromise() || [];

    const idsServicosDev = new Set(this.servicosDesenvolvimento.map(s => s.ServicoID));
    const chamadosDev = chamados.filter(c => c.ServicoID && idsServicosDev.has(c.ServicoID));

    const usuarioMesmoDepartamento = this.usuarios.filter(u => u.DepartamentoID === this.departamentoID);

    this.todosChamadosUsuario = chamadosDev.filter(chamado => {
      const pertenceAoUsuario = chamado.UsuarioCriacaoID === this.usuarioID || chamado.UsuarioResponsavelID === this.usuarioID;
      const pertenceAoDepartamentoResponsavel = this.departamentoResponsavel ? chamado.DepartamentoResponsavelID === this.departamentoResponsavel : false;
      const pertenceAoDepartamentoUsuario = usuarioMesmoDepartamento.some(u => u.UsuarioID === chamado.UsuarioCriacaoID || u.UsuarioID === chamado.UsuarioResponsavelID);
  ...
  protected aplicarFiltro(): void {
    ...
    // 4) Intervalo de datas (inclusivo)
    ...
    // 5) Pesquisa global (usa norm() para remover acentos e case)
    ...
    if (termo) {
      chamadosFiltrados = chamadosFiltrados.filter(c => {
        ...
        const campos = [
          c.Protocolo, c.Titulo, servico, solicitante, responsavel,
          departamento, c.Status, c.Prioridade, dataAbertura
        ];
    ...
    // 6) Ordenação padrão (se nenhuma coluna selecionada)
    ...
    if (!this.destroyed) {
      this.router.navigate([], {
        ...
        queryParams: {
          ...
          pesquisa: this.termoPesquisa || null,
          responsavel: this.responsavelSelecionadoId || null,
  ...
  protected onChangeServico(): void { this.aplicarFiltro(); }
