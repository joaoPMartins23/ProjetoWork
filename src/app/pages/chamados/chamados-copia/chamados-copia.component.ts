...
@Component({
  selector: 'app-chamados-copia',
  templateUrl: './chamados-copia.component.html',
  styleUrl: './chamados-copia.component.scss'
})
export class ChamadosCopiaComponent implements OnInit, OnDestroy {
...
protected async carregarTodosChamados(): Promise<void> {
  ...
  this.todosChamadosUsuario = base.filter((c: any) => {
    const aux = c?.UsuariosAuxiliares;

    if (!Array.isArray(aux) || aux.length === 0) return false;

    // aceita tanto {UsuarioID} quanto variações comuns (segurança)
    return aux.some((x: any) =>
      (x?.UsuarioID || x?.usuarioID || x?.usuarioId) === meuId
    );
  });
}
