import { ClienteService } from './../../../services/cruds/cadastros/cliente.service';
...
@Component({
  selector: 'app-todos-chamados-sac',
  templateUrl: './todos-chamados-sac.component.html',
  styleUrl: './todos-chamados-sac.component.scss'
})
export class TodosChamadosSacComponent implements OnInit, OnDestroy {
  ...
  protected usuariosSolicitantes: IUsuarioView[] = [];
  ...
  protected solicitanteSelecionadoId: string = 'todos';
  protected clienteSelecionadoId: string = 'todos';
  ...
  protected async carregarTodosChamados(): Promise<void> {
    ...
    const chamadosSac = chamados.filter((c: any) => !!c.ClienteID);

    const usuarioMesmoDepartamento = this.usuarios.filter(u => u.DepartamentoID === this.departamentoID);

    this.todosChamadosUsuario = chamadosSac.filter(chamado => {
  ...
  protected aplicarFiltro(): void {
    ...
    // 4) Cliente
    if (this.clienteSelecionadoId && this.clienteSelecionadoId !== 'todos') {
      chamadosFiltrados = chamadosFiltrados.filter(c => c.ClienteID === this.clienteSelecionadoId);
    }
    ...
    if (termo) {
      chamadosFiltrados = chamadosFiltrados.filter(c => {
        ...
        const cliente = this.clientes.find(cl => cl.ClienteID === c.ClienteID)?.NomeCliente || '';
    ...
    let enviados = chamadosFiltrados.map(chamado => ({
      ...
      DataFinalizacao: chamado.DataHoraFinalizacao == "0001-01-01T00:00:00" ? '—' : this.formatarDataHora(chamado.DataHoraFinalizacao),
      ...
      Cliente: this.clientes.find(cl => cl.ClienteID === chamado.ClienteID)?.NomeCliente || 'Desconhecido',
    ...
    if (!this.destroyed) {
      this.router.navigate([], {
        ...
        queryParams: {
          ...
          cliente: this.clienteSelecionadoId,
