...
@Component({
  selector: 'app-chamados-projetos',
  templateUrl: './chamados-projetos.component.html',
  styleUrl: './chamados-projetos.component.scss'
})
export class ChamadosProjetosComponent implements OnInit {
  ...
  protected cabecalhosEnviar: any[] = [
    ...
    { CampoTitulo: "Projeto", CampoChave: "Projeto" },
    { CampoTitulo: "Início Projeto", CampoChave: "DataInicioProjeto" },
    { CampoTitulo: "Estimado", CampoChave: "DataEstimado" },
    { CampoTitulo: "Finalização Projeto", CampoChave: "DataFinalizacaoProjeto" },
  ];
  ...
  protected cabecalhosEditadosEnviar: any[] = [
    ...
    { CampoTitulo: "Projeto" },
    { CampoTitulo: "DataInicioProjeto" },
    { CampoTitulo: "DataEstimado" },
    { CampoTitulo: "DataFinalizacaoProjeto" },
  ];
  ...
  async ngOnInit(): Promise<void> {
    ...
    this.usuariosDepartamento = this.usuarios.filter(u => u.DepartamentoID === this.departamentoID);
  ...
  protected async carregarTodosChamados(): Promise<void> {
    ...
    const usuarioMesmoDepartamento = this.usuarios.filter(
      u => u.DepartamentoID === this.departamentoID
    );

    this.todosChamadosUsuario = chamados.filter(chamado => {
      ...
      const pertenceAoDepartamentoUsuario = usuarioMesmoDepartamento.some(
        u =>
          u.UsuarioID === chamado.UsuarioCriacaoID ||
          u.UsuarioID === chamado.UsuarioResponsavelID
      );

      const temProjeto =
        chamado.ProjetoID !== null &&
        chamado.ProjetoID !== undefined &&
        chamado.ProjetoID !== '';

      return temProjeto && (
        pertenceAoUsuario ||
        pertenceAoDepartamentoResponsavel ||
        pertenceAoDepartamentoUsuario
      );
    });
  }
  ...
  protected aplicarFiltro(): void {
    ...
    if (termo) {
      chamadosFiltrados = chamadosFiltrados.filter(c => {
        ...
        const dataInicioProjeto = c.DataHoraInicioProjeto ? this.formatarDataHora(c.DataHoraInicioProjeto) : '-------------------';
        const dataFinalizacaoProjeto = c.DataHoraFinalizacaoProjeto ? this.formatarDataHora(c.DataHoraFinalizacaoProjeto) : '-------------------';
        const dataEstimado =  c.DataHoraEstimado ? this.formatarDataHora(c.DataHoraEstimado) : '-------------------';
        const projeto = this.projetos.find(p => p.ProjetoID === c.ProjetoID)?.Projeto || '';
    ...
    let enviados = chamadosFiltrados.map(chamado => ({
      ...
      DataInicioProjeto:  chamado.DataHoraInicioProjeto ? this.formatarDataHora(chamado.DataHoraInicioProjeto) : '-------------------',
      DataFinalizacaoProjeto: chamado.DataHoraFinalizacaoProjeto ? this.formatarDataHora(chamado.DataHoraFinalizacaoProjeto) : '-------------------',
      DataEstimado: chamado.DataHoraEstimado ? this.formatarDataHora(chamado.DataHoraEstimado) : '-------------------',
      Projeto: this.projetos.find(p => p.ProjetoID === chamado.ProjetoID)?.Projeto || 'Desconhecido',
