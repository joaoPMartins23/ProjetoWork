...
@Component({
  selector: 'app-chamados-minha-responsabilidade',
  templateUrl: './chamados-minha-responsabilidade.component.html',
  styleUrl: './chamados-minha-responsabilidade.component.scss'
})
export class ChamadosMinhaResponsabilidadeComponent implements OnInit, OnDestroy {
  ...
  async ngOnInit(): Promise<void> {
    ...
    this.activatedRoute.queryParams.subscribe(params => {
      ...
      // Depois do carregamento inicial, sempre que os params mudarem, refiltra
      if (!this.carregando) {
        this.aplicarFiltro();
      }
    });
    ...
    const [_, servicos, usuarios, deps] = await Promise.all([
      this.carregarTodosChamados(), // já popula this.todosChamadosUsuario
    ...
    // 3) Aplica filtro inicial com tudo em memória
    this.aplicarFiltro();
    this.carregando = false;

    // 4) Atualização periódica (1 min)
  ...
  protected async carregarTodosChamados(): Promise<void> {
    ...
    this.todosChamadosUsuario = chamados
      ...
      .filter(ch => ch.UsuarioResponsavelID === this.usuarioID);
  }
  ...
  // Converte o título do cabeçalho para a chave do objeto exibido
